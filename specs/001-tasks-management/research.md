...
This document resolves the technical unknowns surfaced during planning for the ATM tasks management system. Each entry records the decision, rationale, and alternatives considered.

## R1. Storage format and layout

**Decision**: One file-per-record JSON under a `.atm/` store directory, grouped by project. Layout:

```
.atm/
  projects/
    ATM.json              # project record (code, name, labels, counter)
    ATM/
      tasks/
        ATM-0001.json     # one task record (with todos, followups, discussions, history embedded)
        ATM-0002.json
      index.json          # optional, derived; not source of truth
  actors.json             # known actor ids (agents + humans), declared lazily on first use
```

**Rationale**: JSON is plain text, diffs cleanly, and is the lingua franca for agents and CLI tooling. One-file-per-task keeps writes localized (only the touched task file is rewritten), which makes concurrent edits and reviewable diffs straightforward. Embedding todos/followups/discussions/history inside the task record avoids a separate entity store for v1 and matches the YAGNI principle. SQLite was considered (below) and rejected for v1.

**Alternatives considered**:
- **SQLite**: single-file DB with real query power and concurrent writes; rejected for v1 because it is binary (poor diff/review), overkill for the modest scale target (thousands of tasks), and violates the "text storage that version-controls well" constitution principle. Can be revisited if scale forces it.
...
**Decision**: Advisory file locking via `flock`-equivalent (on darwin, `flock(2)` is available via the `golang.org/x/sys/unix` package; on Windows a `LockFileEx` shim). Every store mutation takes an exclusive lock on the project's lock file (`.atm/projects/<CODE>.lock`) for the duration of the read-modify-write. The "next + claim" sequence is a single locked operation: under the project lock, find the next claimable task, mark it claimed, write it, release. Two concurrent claimants therefore never both succeed on the same task; the second sees the updated store and is pointed to the next task or an empty result.
