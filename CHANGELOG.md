...
### feat
- ATM-cdb166: redesigned the Projects-pane activity charts as one combined box with a persona carousel, braille pulse, and `1w`/`1m`/`3m`/`6m`/`1y` time ranges. `Ctrl+Left/Right` changes the persona view, `Ctrl+Up/Down` changes the range, and focused `Enter` opens a read-only drill-in overlay for model, agent, and action usage; `Esc` closes it. Per-persona dispatch and the old bar stripe are removed.
