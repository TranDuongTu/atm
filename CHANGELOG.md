...
### prune
- ATM-bce933 (increment 7, PR 3): the content-migration **prune** lands. **Starter checklists are gone** — `skills/checklist/` and `skills.ChecklistSeeds()` are deleted, `atm capability checklist seed` is slimmed to substrate (vocabulary + board only), the setup wizard's PERSONAS section and its seed/enable ladder are removed, and operating checklists are profile content imported by `atm profile apply` and reported by `atm profile status`. **The concierge persona is deleted** along with its icons, dispatch defaults, and wizard gating — `D` from an empty selection opens on the dialog's own first row, the channels overlay's `c` re-targets to a manager attest dispatch, and the wizard's bootstrap is fully DIRECT (no action waits for a ready agent). **The machine-global persona store is retired**: `atm persona create`/`remove` are gone, `ListPersonas`/`GetPersona` outside a project answer built-ins only, personas go into projects with `atm persona set`, and the dispatch dialog and `V` personas browser list the scoped project's own records first. **`validateActor` is form-only** — persona registration no longer gates mutations (records are project state; an actor string spans projects). **Persona `kickoff`/`expects`/`optional` frontmatter is pruned** (`core.Persona.Kickoff` deleted; `launch` and `project_optional` ride the document), and the dead wizard persona-create form went with the machine-global store.

### fix
- ATM-5caf86: the tasks list's **`id-asc` sort actually sorts by ID**. It was a no-op that leaned on the store's own order, which is id-asc only for a v1 project — there the alias is a zero-padded creation counter. A v2 alias is a content hash, so `ListTasks` orders v2 projects by creation ordinal instead, and cycling `[s]` onto the ID column moved nothing at all. The four `[s]` modes now live in one table pairing each mode's comparator with the column its arrow points at, so a mode cannot sort by one column while marking another.

### feat
- ATM-8b8493: the tasks list gains an **ANNOTATE sort** as the fifth mode in the `[s]` cycle, with the `↑` indicator on the ANNOTATE column header like every other sorted column. Capabilities now rank their own annotations (`capability.Cell.Rank`, computed at annotate time and never persisted), so the sort surfaces the states a reader should look at soonest — an unreadable cell first, then the active stages, then the finished and evicted ones — rather than ordering display text alphabetically. Ranks are per-capability and never compared across capabilities: the sort reads whichever capability the pane is currently showing. Rows the capability ranked come first (lowest rank first), then rows it annotated but did not rank, then rows it said nothing about; within each group the list still reads most-recently-updated first.
- ATM-cdb166: redesigned the Projects-pane activity charts as one combined box with a bounded persona-card carousel, braille pulse, and `1w`/`1m`/`3m`/`6m`/`1y` time ranges. Each visible persona card keeps its icon on screen and summarizes the selected period's activity total plus top three models. `Ctrl+Left/Right` changes the persona view, `Ctrl+Up/Down` changes the range, and Ctrl navigation focuses the chart and keeps the selected persona card highlighted until the chart state resets. `Ctrl+Enter` opens the selected persona's activity breakdown inline inside the chart box; terminals that encode it as plain Enter can focus the chart with `Ctrl+Left/Right` and then press `Enter`, and `Ctrl+J` works as an explicit fallback. The drill-in keeps the carousel pinned at the top and shows the original proportional breakdown rows for models, agents, and actions; scroll it with `j/k`, arrows, or `Ctrl+Up/Down`, use `g` for top, and `Esc` returns to the pulse chart. `Ctrl+Up/Down` changes the time range only when the pulse chart is visible. The active range now appears as a strong, full-English bottom legend, such as `Range: One week`. Per-persona dispatch and the old bar stripe are removed.
- ATM-66a6d2: an **answer engine** over the project's own ledger — `internal/answer`, consumed by no surface yet (the `atm ask` verb and the spotlight's ask mode follow). `Engine.Ask` emits its sources first, then the answer as it streams, then the citations the answer actually earned: retrieval re-runs on every turn (embed the question, `store.Search` top-K, numbered source blocks with a cite-by-number instruction — each block carrying a hit's title and its ~80-character search snippet, not the full task description or comment body, which bounds how much any answer can say), and the sources travel to the consumer before any model is asked to generate, so **hits are delivered even when generation cannot run at all**. No chat model configured, or an endpoint that never produces a single token, ends in a *degraded* answer carrying the hits and the reason — never an error. A break after tokens have arrived is a failure that keeps the partial text, and a canceled ask is told apart from both. New `atm project set-chat --project CODE --model SLUG [--endpoint URL]` declares who answers, borrowing the embedding endpoint when none is given (ollama serves both from one place); with no chat model set, search and indexing are untouched. The new `internal/chat` client streams ollama's OpenAI-compatible `/v1/chat/completions` and is bounded by silence rather than by duration — a 30s idle-delta watchdog that resets on every token, plus a 5-minute total ceiling — because the embedder's whole-request timeout would have truncated any answer that took longer than it to finish.
- ATM-d4ceed: `atm ask "question"`: conversational search over a project's own tasks and comments, streaming in text mode and emitting one complete document in JSON mode. Degraded and interrupted answers both exit 0 with the same shape, so scripts parse one contract.
- ATM-d4ceed: source text sent to the model is now the full task description or comment body, fitted to a character budget, rather than the 80-character snippet the answer engine shipped with. A comment's body is its content; answering from one line of it was the ceiling on answer quality.
- ATM-d4ceed: `atm project set-inquiry-log`: opt a project out of the search/ask inquiry log.
- ATM-d4ceed: `atm inquiry add --returned`: record which IDs a search returned, not only which were cited. Recall@k has no denominator without it.
- ATM-d4ceed: an expired `--timeout` is reported as an interrupted answer rather than a cancellation, so a caller is never told it stopped an answer it did not touch.
- ATM-f71b81: the `\` spotlight gains an **ask level** — `atm ask`'s conversation, over the same retrieval, inside the launcher. `Tab` on a non-empty query (with a project in scope) pushes it over the same box: the turn's retrieved sources down the left, the answer streaming beside them, a follow-up input across the top. `Tab` therefore no longer focuses the preview pane — `→` does, and `←` joins `Esc` as its exit. Retrieval re-runs every turn, so `[1]`…`[8]` number *this* turn's sources and mean what the CLI's cited-sources footer means. `↑/↓` walk the sources and `Enter` opens the highlighted one — closing the launcher onto that task's detail and recording the open in the project's inquiry log as a click-through, kept apart from the model's own citations because a person opening a source is a different judgment from a model citing one. Type a follow-up and `Enter` sends it instead; the prior turns replay as history, minus any turn that generated nothing or that the user canceled (an empty or rejected assistant turn replayed as history poisons every turn after it). `pgup`/`pgdn` and `ctrl+u`/`ctrl+d` scroll the transcript, which follows the tail until you scroll off it and follows again once you catch up; `ctrl+r` retries an answer that broke mid-stream. `Esc` stops a streaming answer, and a second `Esc` peels back to the list with the query, cursor, and drill position intact. The three ways a turn can end stay distinct on screen — *degraded* (sources, no model answer: the hint names `atm project set-chat`, unless chat IS configured, in which case the reason stands alone rather than sending someone to fix a config that is fine), *canceled* (your own `Esc`, no retry offered), and *interrupted* (a disconnect or an expired deadline, `ctrl+r` to retry) — and a partial answer's sources stand in all of them. Deltas coalesce into a builder rather than a droppable channel, because a dropped progress line supersedes and a dropped delta is an invisible hole in the middle of the answer. `\` types into the follow-up input rather than closing the launcher; with no project in scope the Ask row does not appear and `Tab` does nothing, matching what the Task group already says. The exchange lives as long as the level does — the durable conversation is `atm ask --session`.
- ATM-f58753: the `\` spotlight's Task-group results now come from the **store's own search** — the same retrieval path `atm search` takes — rather than from an in-memory matcher of the launcher's own, which is deleted; one rule now explains both surfaces. Typing queries the store on a ~150ms debounce after the last keystroke instead of once per keypress (a superseded tick is dropped rather than published, and the previous results stay on screen through the wait rather than flashing empty), and the corpus is that project's task titles, descriptions, and labels **plus its comments**: hits render as one `TASKS · COMMENTS` section — a row per matching task, with each matching comment nested under the task it was left on, including when the comment's own task did not match, so a hit is never cut loose from what it is about. Up to 8 hits are asked for and the cap falls after the ranking, never before it (the section can therefore carry more than 8 rows, because a nested comment's parent row is synthesized rather than counted against the cap). Hovering a comment row previews the comment — the task it is on, its author, and its full body — and the pane follows the highlight both onto and off the row. `Enter` on a comment opens the same six actions its task row does: a comment is how you found the task, not a thing with actions of its own. A store error is reported as a row rather than as `no tasks match`, and it retires with the query that produced it.
- ATM-f58753: the store's keyword path — the text fallback behind `atm search` as much as the spotlight — gains two rules. It finds an entity by its **ID**: a pasted ID, or a fragment of one, names that entity and ranks it above anything that merely mentions the query's words, which token overlap could not do on its own because an ID is deliberately not part of the document text the scorer reads. And it matches each query word as the **prefix** of a word in the document rather than requiring a whole one, so a half-typed query already has results under it (`ind` finds *wire the indexer*, and so does `indexer wire`); prefix, not infix, so `dex` finds nothing. A query that matches nothing beyond the project-code prefix every ID in the project shares (`a`, `atm`, `atm-`) names no particular entity, so it is scored as the ordinary word match it is instead of ranking the whole project at the tier reserved for a pasted ID.
...
- ATM-77af5e: the Task group is contextual and carries three distinct hint states. With no project in scope: `select a project first`. Scoped to a project with an empty query: `type to find a task…`. Scoped with a query that hits nothing: `no tasks match`. Otherwise, typing searches that project's content — the retrieval rule, the result shape, and the cap that ship are ATM-f58753's, above. Hovering a result previews it — a task's preview includes its history (see the task-detail `H` removal below); `Enter` opens six actions scoped to that task, reachable from any pane, cursor position, or board filter, guarded against acting on a task deleted since the list was built. `Esc` from the task actions returns to the results with the query intact.
