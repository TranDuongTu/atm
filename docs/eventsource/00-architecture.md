...
**Computed labels (boards).** A label carrying an expression (https://app.notion.com/3bc70f5f1db581efa495f89eeffebf61) is a *saved query* over the label substrate — a board. Its expression is one more LWW register in the list above, so it adds no new resolution rule and the interop spec grows by a clause rather than a concept. Two consequences bind L2, and neither is optional:
...
| `03-sync-transport.md` | L4 | Proposed |
