...
- **List ordering**: for a v2 project, `ListTasks`/`ListTasksErr` and `ListComments` order by the entity's creation ordinal (`LogSeq`, stamped by the projector from `TasksByCreation`/`CommentsByCreation`, i.e. the HLC creation order L1 names as the true one), not by id. Under v1 the two coincide — the numeric alias segment IS a zero-padded creation counter — but a v2 alias is a content hash, and a v2 project routinely holds BOTH generations (numeric aliases carried over by the upgrade, hash aliases minted after cutover), so id-asc there interleaves them by hex luck and can print a reply above the comment it answers.
