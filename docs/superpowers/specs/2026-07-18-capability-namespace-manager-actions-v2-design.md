...
**Changed**:
- `EnsureVocabulary` returns `([]core.Label, error)`. It seeds **all** the capability's labels (non-board + boards) into the store idempotently (`LabelSeed` creates absent labels and refreshes capability-owned descriptions without rewriting existing expressions) and returns the **board labels** (those with `Expr`) the capability owns. The capability is fully self-contained — one call leaves the project fully converged for that capability, and the caller gets the boards back.

**Guarantee**: `EnsureVocabulary` is safe to run on any project at any time. It is idempotent, creates missing vocabulary, and refreshes descriptions for labels the capability owns. A new board added in a new capability version appears on the next run (project select in the TUI, TUI Boards re-ensure, `atm project capability add`, `atm project create`, or capability-specific seed commands). No migration step.
...
- `priority:*` + `priority:high/medium/low`: owned by the workflow capability. Priority is a planning concern; workflow is the planning/status capability, so it seeds the `priority:*` namespace descriptor and the three priority values alongside its status vocabulary and boards. (Clarified 2026-07-19: superseded the earlier "no capability owns priority" decision — priority is part of planning hygiene, which workflow's Autopilot already enforces.)
...
5. **`priority:*` is owned by the workflow capability** (2026-07-19 followup; supersedes the original §4 decision that "priority:* is not seeded anywhere") — priority is a planning concern, and workflow is the planning/status capability. Workflow's `EnsureVocabulary` now seeds the `priority:*` namespace descriptor and `priority:high` / `priority:medium` / `priority:low` alongside its status vocabulary and boards. The Autopilot "Planning" scope already enforced `priority:*` hygiene; owning the vocabulary makes that consistent. `comment:*` remains unseeded (invented on demand).
