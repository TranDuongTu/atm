...
| D15 | **The indexer auto-starts on project selection.** When the user selects a project (`s` in the Projects pane sets `projectScope`) AND that project has embedding config, the TUI starts the watcher immediately — no manual `S` required. Re-selecting the same project (or a project whose watcher is already running) is a no-op. Switching to a project with **no** embedding config does **not** start a watcher and does **not** auto-open the overlay — the dock's persistent `⌬ off  g1` hint surfaces the not-configured state without hijacking project selection (the user presses `g 1` to configure when ready). A runtime start *failure* (endpoint down, etc.) is a separate condition handled by D16's error overlay. Supersedes the earlier "out of scope: auto-starting on launch" note — auto-start is now in scope, triggered by project selection rather than launch. |
...
- `autoStartIndexer(code)`: the project-selection entry point (D15). Calls `refreshIndexerStatus()`; if config present and no watcher is running, calls `startIndexer(code)`; if no config, returns without opening the overlay (the dock's `⌬ off  g1` hint surfaces the state). Idempotent: re-selecting a project whose watcher is already running is a no-op.
