...
Three tiers. Tier 1, the ledger record (synced): a task labelled `channel` whose title is the unique handle, whose description is the purpose, and whose `channel` metadata payload carries the endpoints (repo: remote URL; notion: workspace + database/page; slack: workspace slug + channel id). Records written before a channel could have several endpoints carry a `channel:<type>` label instead and read as a single endpoint whose role follows its medium; the first write migrates the label. Addresses are not credentials. Tier 2, local wiring (`config.json`, this machine only, never synced): PER ENDPOINT — the repo clone path, the MCP server name agents here use, and verification stamps recorded when an agent actually reaches that endpoint. A stamp carries the actor, the time, a `kind` (`use` when real work reached it, `probe` when a read-only check did) and a note. The actor already names the agent harness (`persona@AGENT:model`), so "which agents have reached this endpoint" is an aggregation of stamps ATM already writes — no separate record, and normal work keeps attestation fresh by itself. Tier 3 does not exist: ATM stores no tokens, passwords, or credentials, anywhere — authorization lives with the agent's own tooling (e.g. the Notion or Slack MCP server's OAuth store). A channel record carries no permissions or scopes either: a Slack app's bot scopes are granted at install time and its bot is invited to a channel in Slack's own UI, so two machines can share one record while holding different tokens, or none.
...
- `atm channel wire --project <CODE> --name <handle> [--type <t>] [--path <dir>] [--mcp-server <name>]` — record how THIS machine reaches one endpoint. `--type` is optional when the channel has exactly one endpoint and required when it has several: wiring the wrong medium silently is worse than being asked which.
- `atm channel stamp --project <CODE> --name <handle> [--type <t>] [--kind use|probe] --note "..."` — vouch that you actually reached that endpoint just now. Stamps age into the status display, so refresh one after using a channel successfully; `--kind probe` is for a read-only check that reached the endpoint without doing work there.
