...
5. When you have made all the progress you can this session, sweep the project's vocabulary: `atm label list --project <CODE>` against the enabled capabilities' guides. A label no enabled capability owns is either project-curated vocabulary worth a description that says so, or a leftover whose tasks should carry a capability-owned label instead (replace via `atm task label remove` + `atm task label add`). Judging that is your call, not a command's.
