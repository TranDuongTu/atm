...
func TestCapabilityScopeValidation(t *testing.T) {
	h := newGoldenHarness(t)
	// Create with only scrum enabled so qa is known-but-disabled.
	...
	// Known-but-disabled capability (qa is registered but not enabled).
	_, stderr, code = h.run("--persona", "manager", "--project", "ATM", "--agent", "claude", "--capability", "qa")
	if code == ExitSuccess {
	...
	_, _, code = h.run("--persona", "manager", "--project", "ATM", "--agent", "claude", "--capability", "scrum")
	if code != ExitSuccess {
...
func TestSessionContextRendersManager(t *testing.T) {
	...
	for _, want := range []string{"# Persona: manager", "ATM session — ATM"} {
	...
	if !strings.Contains(mcOut, "# Persona: manager") {
		t.Errorf("manage-context output missing persona prompt:\n%s", mcOut)
	}
}
...
func TestSessionTaskAssignment(t *testing.T) {
	...
	if !strings.Contains(string(b), "- Task: `"+taskID+"`") {
		t.Errorf("context missing task line:\n%s", b)
	}
}
