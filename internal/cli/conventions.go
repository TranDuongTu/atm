...
Every log entry and comment carries an actor string of the form ` + "`persona@agent:model`" + ` (e.g. ` + "`staff-engineer@claude:opus-4.8`" + `). Personas are global, human- or agent-authored profiles (name + prompt + description) managed with ` + "`atm persona create|list|show|edit|remove`" + `; the agent segment (` + "`claude`" + `, ` + "`codex`" + `, ` + "`opencode`" + `, ` + "`ollama`" + `) and model are stamped by the host agent itself. An unset or unresolved persona renders as ` + "`(none)`" + `. Start a session with a persona via ` + "`atm developing <agent> --project <CODE> --persona <name>`" + `, which injects the persona's prompt into the session context and defaults the actor to ` + "`<persona>@<agent>`" + ` (an explicit ` + "`--actor`" + ` still wins). Legacy actor strings from before this convention are mapped to personas via ` + "`atm actor migrate [--dry-run]`" + ` (seeds the built-in ` + "`developer`" + `/` + "`manager`" + ` personas and records aliases) and curated by hand with ` + "`atm actor alias set|list|remove`" + `. See who has been doing what with ` + "`atm activity --project <CODE> [--group-by persona|agent|model]`" + `, or the TUI's Projects pane (press ` + "`P`" + ` to expand the \"activity by persona\" chart into an overlay; ` + "`p`" + ` adds a persona).
...
10. To get a synthesized answer, dispatch ` + "`atm-manager`" + ` with ` + "`hint: question`" + `.
...
func conventionsStructured() map[string]any {
	...
	return map[string]any{
		...
		"actor_identity":                    "Every log entry and comment carries an actor string of the form persona@agent:model (e.g. staff-engineer@claude:opus-4.8). Personas are global profiles (name + prompt + description) managed with atm persona create|list|show|edit|remove; the agent segment (claude, codex, opencode, ollama) and model are stamped by the host agent itself. An unset or unresolved persona renders as (none). Start a session with a persona via atm developing <agent> --project <CODE> --persona <name>, which injects the persona's prompt into the session context and defaults the actor to <persona>@<agent> (an explicit --actor still wins). Legacy actor strings are mapped to personas via atm actor migrate [--dry-run] (seeds the built-in developer/manager personas and records aliases) and curated by hand with atm actor alias set|list|remove. See who has been doing what with atm activity --project <CODE> [--group-by persona|agent|model], or the TUI's Projects pane (press P to expand the \"activity by persona\" chart into an overlay; p adds a persona).",
		...
		"agent_first_contact_sequence": []string{
			...
			"to get a synthesized answer, dispatch atm-manager with hint: question",
		},
