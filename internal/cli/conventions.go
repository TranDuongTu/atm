...
func conventionsStructured() map[string]any {
	...
	return map[string]any{
		...
		"workflow_verbs":                    "Status transitions live in the internal/workflow capability, exposed as four mutating verbs: atm workflow start (in-progress), atm workflow open, atm workflow block (blocked), atm workflow complete (done), plus a read-only atm workflow status reporter and atm workflow seed. Each mutating verb swaps the task's status:* label (adds the target, then removes any other), so exactly-one-status is an invariant the capability maintains. The store enforces nothing; raw atm task label add/remove --label <CODE>:status:<value> still works. internal/workflow is a paved road, not a fence — a project can replace it.",
