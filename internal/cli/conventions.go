...
Tasks live in the store, scoped to a project, as entries in the project's append-only log (` + "`log.jsonl`" + ` on v1, ` + "`events.v2.jsonl`" + ` on EventSource v2) — there is no per-task file. ` + "`atm task list --project <CODE>`" + ` lists them. Each task has an ID — ` + "`<CODE>-<NNNN>`" + ` on a v1 project, ` + "`<CODE>-<hex>`" + ` on an EventSource v2 one (an upgraded project keeps its old numeric ids and mints hex ones for new tasks) — a title, a description, and a label set. The description is free-form text — agents write what they are doing, what they found, what they decided, so the next agent or human can pick up where they left off.
...
- ` + "`atm store rollback --project <CODE> --to v1`" + ` — switch an upgraded project back to its preserved v1 log (v2-only writes are not exported back; refused for a project born v2).
...
func conventionsStructured() map[string]any {
	...
	return map[string]any{
		...
		"where_tasks_live":                  "Tasks live in the store, scoped to a project, as entries in the project's append-only log (log.jsonl on v1, events.v2.jsonl on EventSource v2) — there is no per-task file. atm task list --project <CODE> lists them. Each task has an ID — <CODE>-<NNNN> on a v1 project, <CODE>-<hex> on an EventSource v2 one (an upgraded project keeps its old numeric ids and mints hex ones for new tasks) — a title, a description, and a label set. The description is free-form text — agents write what they are doing, what they found, what they decided, so the next agent or human can pick up where they left off.",
