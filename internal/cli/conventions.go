...
First run setup: ` + "`atm init`" + ` initializes the store, installs selected ATM agent plugins, records the default agent, and can save default host-agent args. Then run ` + "`atm manage --project <CODE> --action mapping`" + ` from a repository to let the manager learn and organize the project. Day-to-day development starts with ` + "`atm dev --project <CODE>`" + `. Override the agent for a single launch with ` + "`--agent <name>`" + ` or ` + "`ATM_AGENT`" + `. To pass per-agent flags, append them after ` + "`--`" + ` (e.g. ` + "`atm dev --project ATM -- --yolo`" + `), or set per-agent defaults with ` + "`atm agents args <name> -- <flags>`" + `. Manager work starts with ` + "`atm manage --project <CODE> --curate|--recall|--action <name>`" + `. Use ` + "`atm agents list|select|args`" + ` later to inspect readiness or change agent defaults after setup.
...
func conventionsStructured(descs []capability.Description) map[string]any {
	...
	return map[string]any{
		...
		"agent_first_contact_sequence": []string{
			...
			"read the Capabilities section of this guide, then run each listed capability's guide (atm <command> guide) before operating in its territory. Capabilities own their semantics — how to read the project's knowledge, how to move work through status, whatever else the project enabled — and this document does not restate them.",
			"atm task list --project <CODE> — see the project's tasks; use the boards the label list revealed for the groupings the project actually uses",
