...
10. To get a synthesized answer, run a manager asking session with ` + "`atm manage <agent> --project <CODE> --asking`" + `.
...
- Extra agent args: pass host-agent flags after ` + "`--`" + ` (e.g. ` + "`atm codex --project <CODE> -- --yolo`" + `); defaults may also be set via ` + "`ATM_<AGENT>_ARGS`" + `. ATM passes args through verbatim and does not validate them.
...
func conventionsStructured() map[string]any {
	...
	return map[string]any{
		...
		"agent_first_contact_sequence": []string{
			...
			"to get a synthesized answer, run a manager asking session with atm manage <agent> --project <CODE> --asking",
		},
		"day_to_day_development": "For day-to-day development, start the agent through atm <agent> --project <CODE>. To pass per-agent flags, append them after -- (e.g. atm codex --project ATM -- --yolo). Manager work starts with atm manage <agent> --project <CODE> --planning|--grooming|--tracking|--asking|--glossary|--onboarding.",
