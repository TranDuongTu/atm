...
// Actor validation is form-only (plan §7), so a persona that exists nowhere
// as a record still stamps: "staff" is registered by nothing but the actor
// string itself, which is exactly what these tests need.
