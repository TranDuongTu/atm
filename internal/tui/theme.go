...
type Styles struct {
	...
	PaneActiveStrong lipgloss.Style
...
func buildStyles(themeName ThemeName) Styles {
	...
	s := Styles{
		...
		PaneActiveStrong: lipgloss.NewStyle().Foreground(t.Accent).Bold(true).BorderForeground(t.Accent),
	...
	if themeName == themeMono {
		...
		s.PaneActiveStrong = lipgloss.NewStyle().Bold(true).Reverse(true).BorderForeground(t.Accent)
