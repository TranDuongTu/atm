...
func (e *Engine) UpgradeProject(code string) (*UpgradeReport, error) {
	...
	err := e.WithLock(code, func() error {
		...
		if f, err := e.ProjectFormat(code); err != nil {
			...
			return fmt.Errorf("%w: project %q is already v2-active; upgrade reads from the v1 log and is only legal for v1-active projects", core.ErrConflict, code)
		}
		raw, err := os.ReadFile(e.LogPath(code))
		if err != nil {
			return err
		}
		...
		// and fold.
		snap, err := e.readFileAt(tmp, false)
		if err != nil {
			_ = os.Remove(tmp)
			return err
		}
		...
		if _, err := os.Stat(e.EventsV2Path(code)); err == nil {
			_ = os.Remove(tmp)
			return fmt.Errorf("%w: project %q already has a v2 file", core.ErrConflict, code)
		} else if !os.IsNotExist(err) {
			_ = os.Remove(tmp)
			return err
		}
		if err := os.Rename(tmp, e.EventsV2Path(code)); err != nil {
			_ = os.Remove(tmp)
			return err
		}
		// Project from the state already in hand (no re-read): identical
		// inputs to the pre-carve cacheProjectFromV2State call, folded from
		// the TEMP file we just verified and renamed into place.
		if e.opts.OnProject != nil {
			if err := e.opts.OnProject(code, convertState(code, state, snap.EventCount)); err != nil {
				return err
			}
		}
		...
		// 5/6. Media replaced: the facade wipes the vector indexes (spec
		// L3-15 — their entries are keyed by the v1 log seq, meaningless under
		// v2) and drops any memoized v1 log snapshot (a long-lived TUI must
		// not keep serving pre-cutover ReadLogCached entries across the format
		// switch). Same position and order as the pre-carve wipe+invalidate.
		if e.opts.OnMediaReplaced != nil {
			e.opts.OnMediaReplaced(code)
		}
...
// UpgradeAll upgrades every v1-active project on disk and then flips the
...
func (e *Engine) UpgradeAll() ([]UpgradeReport, error) {
	...
	if err := e.SetActiveFormat(StoreFormatV2); err != nil {
		return out, err
	}
	return out, nil
}
