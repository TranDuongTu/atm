...
func (e *Engine) ProjectCodesOnDisk() ([]string, error) {
...
func (e *Engine) ReadStoreMeta() (*StoreMeta, error) {
	var m StoreMeta
	if err := fsio.ReadJSON(e.storeMetaPath(), &m); err != nil {
		if os.IsNotExist(err) {
...
func (e *Engine) writeStoreMeta(m *StoreMeta) error {
	if m.ProjectFormats == nil {
		m.ProjectFormats = map[string]StoreFormat{}
	}
	now := core.Now()
	...
	m.UpdatedAt = now
	return fsio.WriteJSON(e.storeMetaPath(), m)
}
...
func (e *Engine) MutateStoreMeta(fn func(m *StoreMeta) error) error {
	return e.WithLock(storeMetaLockName, func() error {
...
func (e *Engine) ProjectFormat(code string) (StoreFormat, error) {
	m, err := e.ReadStoreMeta()
	if err != nil {
		return "", err
	}
...
// another process may cut the project over (upgrade) between this read and
// this mutator acquiring the project lock. The body selected here is
// therefore re-checked under the lock by WithProjectFormatLock, which is the
// authoritative gate.
func (e *Engine) DispatchFormat(code string) (StoreFormat, error) {
...
// losing the write, which never reaches events.v2.jsonl). That is silent
// corruption. ErrConflict — which the caller may simply retry — is not.
// UpgradeProjectToV2 has carried exactly this re-check since Task 4; every
...
func (e *Engine) WithProjectFormatLock(code string, want StoreFormat, fn func() error) error {
	return e.WithLock(code, func() error {
		...
		if f != want {
			return fmt.Errorf("%w: project %q changed format (%s -> %s) while this write waited for the project lock; retry", core.ErrConflict, code, want, f)
		}
		return fn()
	})
}

func (e *Engine) SetProjectFormat(code string, f StoreFormat) error {
...
func (e *Engine) RemoveProjectFormat(code string) error {
	return e.MutateStoreMeta(func(m *StoreMeta) error {
...
func (e *Engine) SetActiveFormat(f StoreFormat) error {
	if f != StoreFormatV1 && f != StoreFormatV2 {
		return fmt.Errorf("%w: unknown store format %q", core.ErrUsage, f)
	}
	return e.MutateStoreMeta(func(m *StoreMeta) error {
		if f == StoreFormatV2 {
			...
			for _, code := range codes {
				if _, ok := m.ProjectFormats[code]; !ok {
					return fmt.Errorf("%w: project %q has no explicit format entry; run 'atm store upgrade --all' before setting the active format to v2", core.ErrConflict, code)
				}
			}
		}
		m.ActiveFormat = f
