...
`↑/↓` move the cursor, `Enter` drills into a group (or a task's own action menu), `Esc` peels back one level at a time, and `\` closes the spotlight from any level of the list. `→` focuses the preview pane — from there, arrows or `j`/`k` scroll it a line, `pgup`/`pgdn` a screenful (both page keys also work without switching focus), and `←` or `Esc` hands focus back. Everything else you type filters: a non-empty query flattens the visible list into ranked `Group · Label` matches — at the root, inside a group, or among a task's own actions — and a query with no hits shows a `no matches` hint.

`Tab` **asks** instead of filtering. With a project in scope and something typed, an `Ask ATM: "…"  [Tab]` row pins across the bottom of the box, and `Tab` pushes a conversational level over the same box: the question's retrieved sources down the left, the streaming answer beside them, and a follow-up input across the top. Retrieval re-runs on every turn, so `[1]`…`[8]` always number *this* turn's sources — the same rule the CLI's cited-sources footer follows. `↑/↓` move through the sources and `Enter` opens the highlighted one (which closes the launcher and lands on that task's detail, recording the click-through in the project's inquiry log); type a follow-up and `Enter` sends it instead. `pgup`/`pgdn` or `ctrl+u`/`ctrl+d` scroll the transcript, `ctrl+r` retries an answer that broke mid-stream, and `Esc` stops a streaming answer — a second `Esc` peels back to the list with your query and cursor exactly where you left them. Printable keys, `\` included, type into the follow-up input rather than into the launcher's query. With no chat model configured the sources still arrive and a hint names `atm project set-chat`; a project with none in scope has nothing to retrieve from, so the row does not appear and `Tab` does nothing. The whole exchange lives for as long as the level does — the durable conversation is `atm ask --session`.

The **Task** group turns into a live search once a project is in scope, and what answers it is the store's own search — the same retrieval path `atm search` takes, so one rule explains both surfaces. Typing queries it on a ~150ms debounce after the last keystroke rather than once per keypress, and the corpus is that project's task titles, descriptions, and labels **plus its comments**: results arrive as one `TASKS · COMMENTS` section, a row per matching task with each matching comment nested under the task it was left on (a comment whose own task didn't match still renders under that task, so a hit is never cut loose from what it's about). Up to 8 hits are asked for, and the cap falls after the ranking rather than before it. Matching is case-insensitive and per word: every word you've typed must start a word somewhere in the task, so `the index` and `indexer wire` both find *Wire The Indexer* while `dex` finds nothing. A pasted ID — or a fragment of one — names that entity and ranks it above anything that merely mentions your words; a query that matches nothing beyond the project-code prefix every ID shares (`a`, `atm`, `atm-`) names nobody and is scored as the ordinary word match it is. Hovering a result previews it: a task row shows its labels, description, and audit history, and a comment row shows the task it's on, its author, and its full body. `Enter` on either opens a menu of six actions scoped to that one task (edit title, edit description, add/remove label, comment, remove) reachable from wherever you currently are in the app — a comment is how you found the task, not a thing with actions of its own.
